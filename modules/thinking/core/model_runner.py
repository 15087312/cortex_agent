        ...
        try:
            ...
            if _cfg.effective_execution_mode == "plan":
                ...
                base_prompt += (
                    ...
                    "5. 全部步骤验证成功后调 save_recipe(name, app_name, description) 保存\n\n"
                    "键盘工具参数：\n"
                    "- keyboard_press(key='enter') — 按单个键\n"
                    "- keyboard_hotkey(keys=['command', 'l']) — 组合键（参数是 keys 列表）\n"
                    "- keyboard_type(text='真实文本') — 输入文字\n\n"
                    "保存后审查（让工具更灵活）：\n"
                    "1. view_recipe(tool_name) — 查看保存的步骤和参数\n"
                    "2. 把固定文本改为 {{变量名}}，如 '今天的天气' → '{{query}}'\n"
                    "3. edit_recipe(tool_name, step_edits=[{step_index:1, args:{text:'{{query}}'}}], params_schema=...) — 应用修改\n"
                    "4. 现在可以传参调用了: tool_name(query='新内容')\n\n"
                    "关键规则：\n"
                    "- 每一步操作后必须调用 understand_screen() 验证操作是否成功\n"
                    "- 验证时观察屏幕是否发生了预期变化（如页面跳转、文本出现等）\n"
                    "- 如果验证发现操作未成功，重新执行该步骤\n"
                    "- 只有验证确认操作成功后才继续下一步\n"
                    "- 所有步骤都验证成功了才调 save_recipe\n"
