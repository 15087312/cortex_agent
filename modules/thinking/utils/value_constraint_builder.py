...
from modules.memory.core.personality import PersonalityMemory
...
class ValueConstraintBuilder:
    ...
    def __init__(self, personality: PersonalityMemory = None):
        self.personality = personality or PersonalityMemory()
    ...
    def build_behavior_rules(self) -> str:
        """构建行为规则"""
        rules = self.personality.get_behavior_rules()
    ...
    def build_value_weights(self) -> str:
        ...
        告诉模型各价值观的重要性
        """
        values = self.personality.get_values()
        ...
        constraints = []
        constraints.append("")
        constraints.append("【价值观权重 - 参考执行】")
        constraints.append("（数值越高越优先，1.0为最高）")
        ...
        for name, weight in sorted_values:
            ...
            stars = "★" * int(weight * 5)
            constraints.append(f"- {name_cn} ({name}): {stars} {weight:.0%}")
        
        return "\n".join(constraints)

    def build_speaking_constraints(self) -> str:
        """构建说话风格约束"""
        style = self.personality.get_speaking_style()
        
        traits = self.personality.get_trait("personality.traits", [])
        tone = self.personality.get_trait("personality.tone", "专业但友好")
        
        constraints = []
        constraints.append("")
        constraints.append("【说话风格约束】")
        constraints.append(f"语气：{tone}")
