...
def get_som_labeled_img(image_source: Union[str, Image.Image], model=None, BOX_TRESHOLD=0.01, output_coord_in_ratio=False, ocr_bbox=None, text_scale=0.4, text_padding=5, draw_bbox_config=None, caption_model_processor=None, ocr_text=[], use_local_semantics=True, iou_threshold=0.9,prompt=None, scale_img=False, imgsz=None, batch_size=128):
    ...
    if use_local_semantics:
        ...
        ocr_text = [f"Text Box ID {i}: {txt}" for i, txt in enumerate(ocr_text)] if ocr_text else []
...
def check_ocr_box(image_source: Union[str, Image.Image], display_img = True, output_bb_format='xywh', goal_filtering=None, easyocr_args=None, use_paddleocr=False):
    ...
    if use_paddleocr:
        ...
        if isinstance(result, dict):
            coord = [p for p, s in zip(result.get('rec_polys', []), result.get('rec_scores', [])) if s > text_threshold]
            text = [t for t, s in zip(result.get('rec_texts', []), result.get('rec_scores', [])) if s > text_threshold]
        else:
